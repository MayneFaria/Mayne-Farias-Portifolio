    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {projects.map((project, index) => (
          <Card key={index} className="bg-pink-50 border-pink-200">
            <CardHeader>
              <CardTitle className="text-pink-700">{project.title}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-pink-600">{project.description}</p>
              <a href={project.link} className="text-pink-500 hover:text-pink-700 hover:underline mt-2 inline-block">Ver projeto</a>
            </CardContent>
          </Card>
        ))}
      </div>
